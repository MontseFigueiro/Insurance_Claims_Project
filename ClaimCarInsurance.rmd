...
```
##Variables Años
A partir del año de la póliza y del año del vehículo podemos extraer la edad del mismo como una variable adicional.

train$añocoche <- ifelse(train[,5]>train[,4],1,0)
sum(train$añocoche)
head(train[train$añocoche==1,])

edad_coche <-((train[,4]-train[,5]))
mean <- mean(edad_coche)
sd <- sd(edad_coche)
norm <- (edad_coche - mean)/sd
...
###Imputacción de los Missing Value con MICE

```{r}
train_mice <- train[,9:35]
muestra <- train_mice[1:10000,]

head(muestra)
imp <- mice(muestra, m=5,method= "polyreg", seed = 1234)
train$OrdCat <- as.factor(train$OrdCat)
