...
```{}
library(data.table)
train <- fread("train_set.csv")test <- read.csv("test_set.csv")
str(train)
summary(train)
```
...
str(train)
```
Podemos prescindir de las variables "Blind_Make" y "Blind_Model" porque están incluidas dentro de "Blind_Submodel":

```{r}
train$make <- lapply(strsplit(as.character(train$Blind_Submodel), "\\."), "[", 2)
agregadosubmodelo <- aggregate(Blind_Submodel~Blind_Submodel+Cat1+Cat2+Cat3+Cat4+Cat5+Cat6+Cat7+Cat8+Cat9,train,length, na.action = na.omit)
...
```{r}
train$EdadVehiculo <- (train$Calendar_Year-train$Model_Year)
```
¿normalizamos la edadvehiculo?

Vemos que hay valores negativos en la edad del vehículo, es decir que si el año de la póliza es 2005 el año del modelo es 2006
```{r}
