...
Si elimino Cat7 que es la variable que tiene 7.167.634 NA's: (los casos completos aumentan hasta 4.575.497 pero no sabemos si eliminarla puesto que puede ser significativa para el modelo)
...
```{r}
positiveclaim <- Data[Data$Claim_Amount>0,]
```
...
```{r}    
casos_completos <- Data[complete.cases(Data),]
```
...
dput(names(casos_completos))
cols <- c("Blind_Submodel", "Model_Year", "Cat1", "Cat2", "Cat3", 
...
"Claim_Amount", "classification")
casos_completos <- casos_completos[,cols]

prop.table(table(casos_completos$classification))
```    
