...
agregado_modelos_test <-aggregate(count~Model+Cat1+Cat4+Cat5+Cat6+Cat7,modelos_test,sum)
agregado_modelos_test$count <- as.numeric(agregado_modelos_test$count)
duplicated <- anyDuplicated(agregado_modelos_test$Model)
agregado_modelos_test[agregado_modelos_test$Model=="Z.27.21999",]
d <- pruebamerge[pruebamerge$Model=="Z.27.21999",]
head(d)
head(agregado_modelos_test[order(-agregado_modelos_test[,7]),])
...
pruebamerge <- merge(x = train, y = agregado_modelos_test , by = "Model", all.x = TRUE)
casos_coincidentes <- pruebamerge[!is.na(pruebamerge$count),]
dim(casos_coincidentes)
