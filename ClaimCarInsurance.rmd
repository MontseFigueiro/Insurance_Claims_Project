...
imp2 <- mice(data=submodels, m=5,maxit=5,method= "polyreg",MaxNWts = 2000)
imp3 <- mice(data=submodels, m=3,maxit=50,method= "polyreg",MaxNWts = 2000)
modelFit <- with(imp,lm(Claim_Amount~ Cat1+Cat2+Cat3+Cat4+ Cat5+ Cat6+Cat7+Cat8+Cat9))
...
traindf1 <- train[1:7000000,]
traindf2 <- train[7000001:13184290,]

traindf1 <- merge(traindf1,datadf,by="ID",all.x = TRUE)
traindf2 <- merge(traindf2,datadf,by="ID",all.x = TRUE)

na <- traindf[is.na(traindf$Cat1m),]
```



si agrupamos house+submodel+model_year nos da cuantas polizas diferentes tenemos en el dataset "train", muchas observaciones no son independientes se trata del mismo vehículo asegurado en diferentes años, pero hay variables que no se mantienen constantes (como Cat10, Cat11, Cat12):
