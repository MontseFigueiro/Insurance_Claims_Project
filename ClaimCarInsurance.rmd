...
```
ROSE para hacer el resampling de la base de datos, ,por defecto utiliza el método Both, una mezcla de Over and Under.
```{r}
library(ROSE)
...
correlationmatrix <- cor(train_ROSE[,17:24])
correlationmatrix
sample <- train_ROSE[sample(nrow(train_ROSE), 100000), ]
highlyCorrelated <- findCorrelation(correlationmatrix, cutoff=0.5)
highlyCorrelated
control <- trainControl(method="repeatedcv", number=10, repeats=3)
# train the model
model <- train(classification~., data=sample, method="lvq", preProcess="scale",trControl=control)
...
```{r}
summary(train_ROSE)
chisq.test(train_ROSE$Cat1,train_ROSE$Cat12)
chisq.test(train_ROSE$Blind_Submodel,train_ROSE$Cat1)
chisq.test(train_ROSE$Blind_Submodel,train_ROSE$Cat2)
...
pruebahetcor <- train_ROSE[,c("Cat1","Cat2","Cat3","Cat4","Cat5","Cat6","Cat7","Cat8","Cat9","Cat10","Cat11","Cat12","OrdCat")]
#Interpretación 
...
library(car)#call vif
prueba <- casos_completos[,8:36]
head(casos_completos)
model <- lm(Claim_Amount~Cat10+Cat11+Cat12,casos_completos)
model <- glm(NVVar4~NVVar1+NVVar2+NVVar3,prueba)
model <- glm(Claim_Amount~Cat10+Cat11+Cat12+Var1+Var2+Var3+Var4+Var5+Var6+Var7+Var8,family=poisson,prueba)

model <- lm(Var4~Var1+Var2+Var3+Var5+Var6+Var7+Var8,prueba)
model <- lm(Var3~Var5+Var1+Var4+Var2+Var6+Var7+Var8,prueba)
model <- lm(Var6~.,prueba)
model <- lm(Var7~.,prueba)
model <- lm(Var8~.,prueba)
pruebanum <- prueba[,15:22]
cor(pruebanum)
summary(model)
Rsq = summary(model)$r.squared
Rsq
1/(1-Rsq)
unique(casos_completos$Var5)
```
