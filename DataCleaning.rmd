...
Eliminamos las observaciones repetidas, el mismo coche puede estar asegurado en distintos años, lo dejamos como una observación única:
```{r}
prueba <- train
prueba$clasification <- as.factor(ifelse(prueba$Claim_Amount==0,"0","1"))
train_household <- aggregate(Claim_Amount~Household_ID+Blind_Submodel+Model_Year+Cat1+Cat2+Cat3+Cat4+Cat5+Cat6+Cat7+Cat8+Cat9+OrdCat+Var1+Var2+Var3+Var4+Var5+Var6+Var7+Var8+NVCat+NVVar1+NVVar2+NVVar3+NVVar4+clasification,prueba,sum)
```
Vamos a agregar Claim_Amount, calculando el Total, la media y el número de observaciones con importe por daño corporal y sin importe, para cada observación, para ello no vamos a tener en cuenta el año de la póliza, ya que existen vehículos repetidos
```{r}
train_reducido <- aggregate(Claim_Amount~Blind_Submodel+Model_Year+Cat1+Cat2+Cat3+Cat4+Cat5+Cat6+Cat7+Cat8+Cat9+OrdCat+Var1+Var2+Var3+Var4+Var5+Var6+Var7+Var8+NVCat+NVVar1+NVVar2+NVVar3+NVVar4+clasification,train_household,FUN = function(x) c(TotalAmount = sum(x), freqAmount = length(x), MediaAmount=mean(x) ))
train_reducido$TotalAmount <- train_reducido$Claim_Amount[,1]
train_reducido$FreqAmount <- train_reducido$Claim_Amount[,2]
train_reducido$MediaAmount <- train_reducido$Claim_Amount[,3]
train_reducido$Claim_Amount <- NULL
head(train_reducido[order(-train_reducido$TotalAmount),])
```
...
head(traindf[order(-traindf$TotalAmount),])
head(traindf[order(-traindf$FreqAmount),])
head(traindf[order(-traindf$MediaAmount),])
...
head(testdf[order(-testdf$TotalAmount),])
head(testdf[order(-testdf$FreqAmount),])
head(testdf[order(-testdf$MediaAmount),])
```
